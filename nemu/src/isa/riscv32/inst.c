  ...
  ***************************************************************************************/
  ...
  enum {
    TYPE_I, TYPE_U, TYPE_S, TYPE_J, TYPE_R, TYPE_B, TYPE_M, TYPE_CSR, TYPE_CSRI,
    TYPE_N, // none
  };
  ...
  #define immCSRI() do { *imm = BITS(i, 31, 20); *src1 = BITS(i, 19, 15); } while(0)
  ...
  static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
    ...
    *rd     = BITS(i, 11, 7);
    switch (type) {
      ...
      case TYPE_B: immB(); src2R(); src1R(); break;
      case TYPE_M: src1R(); src2R();         break;
      case TYPE_CSR: immCSR(); src1R();      break;
      case TYPE_CSRI: immCSRI();              break;
    }
  }
  ...
  #define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = ((imm+src1)&(0xfffffffe)); R(rd) = s->pc+4;
    IFDEF(CONFIG_FTRACE, ftrace_jalr(s->pc, s->dnpc, rd, BITS(s->isa.inst.val, 19, 15))));
    ...
    INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = C(0x341));

    /*M-Extention*/
    INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , M, R(rd) = src1*src2);
    INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , M, R(rd) = ((int64_t)(sword_t)src1*(int64_t)(sword_t)src2)>>32);
    INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , M, R(rd) = ((int64_t)src1*(uint64_t)src2)>>32);
    INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , M, R(rd) = ((uint64_t)(sword_t)src1*(uint64_t)src2)>>32);

    INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , M, R(rd) = (sword_t)src1/(sword_t)src2);
    INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , M, R(rd) = src1/src2);

    INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , M, R(rd) = (sword_t)src1%(sword_t)src2);
    INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , M, R(rd) = src1%src2);

    /*CSR Instructions*/
    INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , CSR, R(rd) = C(imm); C(imm) = src1);
    INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , CSR, R(rd) = C(imm); C(imm) = C(imm) | src1);
    INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , CSR, R(rd) = C(imm); C(imm) = C(imm) & ~src1);
    INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , CSRI, R(rd) = C(imm); C(imm) = src1);
    INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , CSRI, R(rd) = C(imm); C(imm) = C(imm) | src1);
    INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , CSRI, R(rd) = C(imm); C(imm) = C(imm) & ~src1);
