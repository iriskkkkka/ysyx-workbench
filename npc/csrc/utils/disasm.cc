...
extern "C" void init_disasm(const char *triple) {
  ...
  std::string errstr;
  llvm::Triple gTriple(llvm::Triple::normalize(triple));  // normalize + construct Triple

  llvm::MCInstrInfo *gMII = nullptr;
  llvm::MCRegisterInfo *gMRI = nullptr;

  const llvm::Target *target = llvm::TargetRegistry::lookupTarget(gTriple, errstr);
  if (!target) {
    llvm::errs() << "Can't find target for " << gTriple.str() << ": " << errstr << "\n";
    assert(0);
  }
